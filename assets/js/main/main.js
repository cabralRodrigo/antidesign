  ...
  init: function () {
    if (document.querySelector(".JS__cards-carousel")) {
      ...
      $(".JS__cards-carousel").each(function (index, value) {
        ...
        let classAdd = str1 + i,
        classCall = str2 + i, 
        next_slider = "slider-next" + i,
        pagination = "slider-pagination" + i,
        prev_slider = "slider-prev" + i;
...
var searchBox = {
  ...
  changePassageiros: function () {
    const inputsPassageiros = document.querySelectorAll(
        ".JS__search-box__quantity"
      ),
  ...
  passageiros: function () {
    ...
    $(document).on(
      "click",
      ".JS__search-box__quantity-plus",
      searchBox.changePassageiros
    );
    $(document).on(
      "click",
      ".JS__search-box__quantity-minus",
      searchBox.changePassageiros
    );
  ...
  removeLastRoom: function () {
    ...
    if (total > 1) {
      $(".JS__passageiros-box__content")
        .children(".JS__hotel-room")
        .last()
        .remove();

      var total =
          parseInt(document.querySelector(".JS__search-box__quartos").value) -
          1,
        quartos = document.querySelector(".JS__total-quartos");
  ...
  addRoom: function () {
    ...
    var new_row =
      ...
      "<span class='fw-500 fs-12 mb-3 d-block'>Quarto <span class='JS__hotel-room__quantity'>" +
      new_chq_no +
      "</span></span>" +
    ...
    var total =
        parseInt(document.querySelector(".JS__search-box__quartos").value) + 1,
      quartos = document.querySelector(".JS__total-quartos");
    quartos.textContent = total.pad();
  ...
  addChild: function (e) {
    var new_child =
      `<div class="mt-3 o-search-box__form__passageiros__content__room__child-age JS__child-age">` +
  ...
  init: function () {
    if (document.querySelector(".JS__tabsFlex")) {
      $(".JS__tabsFlex").each(function (index, value) {
        ...
        if (($(this).find(".nav-item").length == 2)||($(this).find(".nav-link").length == 2))
...
const openPassageiros = () => {
  ...
  setTimeout(() => {
    document.addEventListener("click", closePassageiros, false);
  }, 1);
};
...
const closePassageiros = (event) => {
  ...
  if (
    !passageirosBox.contains(event.target) &&
    !event.target.classList.contains("JS__room-remove")
  ) {
    searchBox.changePassageiros();
...
const searchboxDateOpen = () => {
  const dateBox = document.querySelector(
    ".JS__searchBox-data .flatpickr-calendar"
  );
  ...
  setTimeout(() => {
    document.addEventListener("click", searchboxDateClose, false);
  }, 1);
};
...
const openSearchboxReducedRadios = () => {
  ...
  setTimeout(() => {
    document.addEventListener("click", closeSearchboxReducedRadios, false);
  }, 1);
};
